...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-gray-100 font-sans pb-24 selection:bg-primary/30 antialiased overflow-x-hidden">
      {/* Printable Area */}
      {printTicketData && (
      ...
      <div className="max-w-4xl mx-auto px-4">
        <header className="py-4 md:py-6 flex justify-between items-center sticky top-0 bg-[#0a0a0a] z-50">
            <div className="flex items-center gap-2">
                ...
                <span className="text-lg md:text-2xl font-black tracking-tighter">SPORT<span className="text-blue-600">POS</span></span>
            </div>
            <div className="flex items-center gap-2 md:gap-4">
                <button
                    onClick={() => axios.post(`${API_URL}/refresh`).then(() => fetchEvents(selectedSport))}
                    className="p-2 md:p-3 bg-white/5 rounded-xl md:rounded-2xl hover:bg-white/10 transition-all border border-white/5 active:scale-95"
                >
                    <RefreshCw size={18} className={loading ? 'animate-spin text-blue-500' : 'text-gray-400'} />
                </button>
                <div className="bg-green-500/10 border border-green-500/20 px-4 md:px-6 py-1.5 md:py-2 rounded-xl md:rounded-2xl">
                    <span className="text-green-500 font-mono font-black text-base md:text-xl tracking-tighter italic">
        ...
        <main>
                ...
                <div className="space-y-6">
                    <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
                        {sports.map(s => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-xl text-xs md:text-sm font-black transition-all border whitespace-nowrap ${
                                    selectedSport === s.key
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-24 gap-4 opacity-30">
                            ...
                            <span className="font-black text-[10px] tracking-widest uppercase text-center">Consultando Cuotas...</span>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {events.map(ev => (
                                <div key={ev.id} className="bg-[#141414] rounded-3xl p-5 md:p-6 border border-white/5 shadow-2xl">
                                    ...
                                    <h3 className="text-center font-black text-sm md:text-base mb-6 tracking-tight px-2 leading-tight">
                                        {ev.home_team} <span className="text-blue-600/40 mx-1 italic font-serif">vs</span> {ev.away_team}
                                    </h3>

                                    <div className="space-y-5">
                                        {odds[ev.id]?.h2h && (
                                            <div className="grid grid-cols-3 gap-2">
                                                <MarketBtn label="L" val={odds[ev.id].h2h.home} onClick={() => setBetSlip({id: ev.id, name: `${ev.home_team} vs ${ev.away_team}`, market: 'ML', sel: ev.home_team, price: odds[ev.id].h2h.home})} />
                                                ...
                                                <MarketBtn label="V" val={odds[ev.id].h2h.away} onClick={() => setBetSlip({id: ev.id, name: `${ev.home_team} vs ${ev.away_team}`, market: 'ML', sel: ev.away_team, price: odds[ev.id].h2h.away})} />
                                            </div>
                                        )}
            ...
            {activeTab === 'tickets' && (
                <div className="animate-in slide-in-from-right duration-500 max-w-lg mx-auto">
                    ...
                    <div className="space-y-3">
                        {history.map(bet => (
                            <div key={bet.id} className="bg-[#141414] p-5 rounded-[2rem] border border-white/5 flex justify-between items-center shadow-xl">
                                <div className="flex-1 min-w-0 pr-4">
                                    <div className="text-[9px] font-black text-gray-600 mb-1 tracking-tighter">ID: {bet.ticket_id}</div>
                                    <div className="font-bold text-sm truncate leading-tight mb-0.5">{bet.event_name}</div>
                                    <div className="text-[11px] font-black text-blue-500 uppercase tracking-wide">{bet.selection} <span className="text-gray-600 italic">@ {bet.odds}</span></div>
                                </div>
                                <div className="text-right">
                                    <div className={`text-[9px] font-black px-2.5 py-0.5 rounded-full inline-block mb-1.5 ${
            ...
            {activeTab === 'caja' && (
                <div className="animate-in slide-in-from-right duration-500 max-w-lg mx-auto">
                    <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-[2.5rem] p-8 md:p-10 mb-8 shadow-2xl shadow-blue-600/30 relative overflow-hidden group">
                        ...
                        <div className="relative z-10 text-center md:text-left">
                            <span className="text-white/60 text-[10px] font-black tracking-[0.2em] mb-2 block uppercase">Balance Disponible</span>
                            <div className="text-4xl md:text-5xl font-black mb-8 tracking-tighter font-mono italic">${balance.toLocaleString(undefined, {minimumFractionDigits:2})}</div>
                            <div className="grid grid-cols-2 gap-3">
                                <button onClick={handleSettle} className="bg-white text-blue-700 py-3.5 rounded-2xl font-black text-xs active:scale-95 transition-all shadow-xl uppercase">Liquidar</button>
                                <button onClick={() => {
                                    const a = prompt('Monto del depósito:');
                                    if(a) axios.post(`${API_URL}/deposit?amount=${a}`).then(fetchBalance);
                                }} className="bg-black/20 text-white py-3.5 rounded-2xl font-black text-xs border border-white/10 backdrop-blur-md active:scale-95 transition-all uppercase">Depósito</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#111] rounded-[2rem] border border-white/5 overflow-hidden shadow-2xl">
                        {transactions.map(tx => (
                            <div key={tx.id} className="p-4 border-b border-white/5 flex items-center justify-between last:border-0 hover:bg-white/[0.02] transition-colors">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="text-[11px] font-black text-gray-200 leading-tight">{tx.description}</div>
            ...
            <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[60] flex items-end animate-in fade-in duration-300">
                <div className="w-full max-w-xl mx-auto bg-[#141414] rounded-t-[3rem] p-8 md:p-10 border-t border-white/10 shadow-2xl animate-in slide-in-from-bottom duration-500">
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <button onClick={() => setBetSlip(null)} className="p-3 bg-white/5 rounded-full hover:bg-white/10 transition-all"><X size={20}/></button>
                    </div>

                    <div className="bg-white/5 rounded-3xl p-6 mb-8 border border-white/5 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Ticket size={64}/></div>
                        <div className="text-gray-500 text-[9px] font-black mb-1.5 uppercase tracking-widest">{betSlip.name}</div>
                        <div className="text-blue-500 text-[10px] font-black tracking-[0.3em] mb-2">{betSlip.market}</div>
                        <div className="flex justify-between items-end gap-4">
                            <div className="text-xl font-black text-white uppercase leading-tight max-w-[60%]">{betSlip.sel}</div>
                    ...
                    <div className="grid grid-cols-4 gap-3 mb-8">
                        {[10, 20, 50, 100].map(v => (
                            <button key={v} onClick={() => placeBet(v)} className="py-4 bg-white/5 rounded-2xl font-black text-sm border border-white/5 active:bg-blue-600 active:text-white transition-all shadow-lg">
                                ${v}
                            </button>
                        ))}
                    </div>
        ...
        {printTicketData && (
            <div className="fixed inset-0 bg-black z-[100] flex flex-col items-center justify-center p-6 no-print">
                <div className="bg-white text-black p-8 rounded-[2rem] shadow-2xl w-full max-w-[320px] font-mono text-xs mb-8 border-2 border-blue-600 animate-in zoom-in-95 duration-300">
                     <div className="text-center border-b border-black pb-3 mb-3">
                        <h2 className="text-xl font-black italic tracking-tighter">SPORTPOS</h2>
                        <div className="text-[10px] opacity-60 italic">{new Date().toLocaleString()}</div>
                     </div>
                     <div className="space-y-2 mb-4">
                        <div className="font-black text-center text-sm uppercase leading-tight mb-2">{printTicketData.event_name}</div>
                        <div className="flex justify-between border-b border-gray-100 pb-1">
                            <span>OPCIÓN:</span>
                            <span className="font-bold">{printTicketData.selection}</span>
                        </div>
                        <div className="flex justify-between text-lg font-black pt-2 text-blue-700">
                            <span>PREMIO:</span>
                            <span>${printTicketData.potential_payout.toFixed(2)}</span>
                        </div>
                     </div>
                     <div className="text-center text-[9px] font-bold opacity-30 uppercase tracking-widest italic">Ticket: {printTicketData.ticket_id}</div>
                </div>
                <div className="flex gap-3 w-full max-w-[320px]">
                    <button onClick={handlePrint} className="flex-1 py-4 bg-blue-600 rounded-2xl font-black text-base flex items-center justify-center gap-2 active:scale-95 transition-all shadow-xl shadow-blue-600/30">
                        <Printer size={20}/> IMPRIMIR
                    </button>
                    <button onClick={() => setPrintTicketData(null)} className="flex-1 py-4 bg-white/10 rounded-2xl font-black text-base active:scale-95 transition-all border border-white/10">CERRAR</button>
                </div>
            </div>
        )}
        ...
        <nav className="fixed bottom-4 left-4 right-4 flex justify-center z-50 pointer-events-none">
            <div className="bg-[#1a1a1a]/90 backdrop-blur-2xl border border-white/5 rounded-2xl px-2 py-1.5 flex gap-1 shadow-2xl shadow-black/80 pointer-events-auto">
...
function MarketBtn({ label, val, sub, onClick }) {
    if (!val) return <div className="bg-white/[0.01] border border-white/[0.02] rounded-xl h-12 flex items-center justify-center opacity-10"><span className="text-[8px] font-black">-</span></div>;
    return (
        <button onClick={onClick} className="w-full bg-white/5 border border-white/5 hover:border-blue-600/50 active:bg-blue-600/10 rounded-xl p-2.5 flex flex-col items-center justify-center gap-0 active:scale-95 transition-all group overflow-hidden relative">
            {sub && <div className="text-[6px] font-black text-gray-600 absolute top-1 left-2 tracking-tighter truncate w-3/4 text-left group-hover:text-blue-500/50 transition-colors uppercase">{sub}</div>}
            <div className="text-[8px] font-black text-gray-500 tracking-widest group-hover:text-blue-400 transition-colors uppercase">{label}</div>
            <div className="text-base md:text-lg font-black font-mono tracking-tighter group-hover:scale-105 transition-transform italic">{val}</div>
        </button>
    );
}
...
function NavBtn({ icon, active, label, onClick }) {
  return (
    <button onClick={onClick} className={`flex flex-col items-center px-4 py-2 rounded-xl transition-all ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/40 scale-105' : 'text-gray-500 hover:text-gray-300'}`}>
